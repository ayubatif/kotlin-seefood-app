...
import android.widget.Toast import com.google.firebase.auth.FirebaseAuth import com.google.firebase.firestore.FirebaseFirestore
...
lateinit var gridContent: Context
 ...
 *
 */
class FavoritesViewer : Fragment() {
    // TODO: Rename and change types of parameters
    private var db = FirebaseFirestore.getInstance()
    private var param1: String? = null
    private var param2: String? = null
    private var listener: OnFragmentInteractionListener? = null
    private var adapter: FavoriteGridAdapter? = null
    ...
    override fun onCreateView(
        ...
        var foodsRef = db.collection("Users").document(auth.currentUser!!.uid).collection("Favorites")
        foodsRef.orderBy("ID").limit(6).get()
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    for (document in task.result!!) {
                        i++
                        //val foodID = foodLocation.document(document.data.get("ID").toString())
                        val foodID = document.data.get("ID").toString()
                        val foodLocation = db.collection("Foods").document(foodID)
                        foodLocation.get().addOnSuccessListener { documentSnapshot ->
                            val foodGridName = documentSnapshot.data?.get("FoodName").toString()
                            val foodGridImageUrlString = documentSnapshot.data?.get("ImageUrl").toString()
                            //var foodGridImageUrlBitmap = BitmapFactory.decodeStream(foodGridImageUrlString.openConnection().getInputStream())
                            val foodGridItem: FoodGrid = FoodGrid(foodGridName, foodGridImageUrlString)
                            foodGridList.add(foodGridItem)
                            foods.add(documentSnapshot.data)
                            foodIds.add(documentSnapshot.id)
                            if(i == task.result!!.size() ){
                                adapter = FavoriteGridAdapter(gridContent, foodGridList)
                                ...
                                foodGridView.findViewById<GridView>(R.id.food_grid).setOnItemClickListener{
                                    ...
                                    Toast.makeText(gridContent,"Clicked Item: "+ foodGridList[position].foodName,Toast.LENGTH_SHORT).show()
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance() =
            FavoritesViewer().apply {
                arguments = Bundle().apply {
                }
            }
    }
}
class FavoriteGridAdapter : BaseAdapter {
